...
if __name__ == '__main__':
    # Cleanup old analysis files on startup
    try:
        from hierarchy_analyzer import HierarchyAnalyzer
        dummy_client = JiraClient('dummy', 'dummy')
        analyzer = HierarchyAnalyzer(dummy_client)
        analyzer.cleanup_old_analyses(days_old=7)
    except Exception as e:
        logger.warning(f"⚠️ Failed to cleanup old analyses: {str(e)}")
