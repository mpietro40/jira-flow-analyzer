...
Version: 2.0.0 - Refactored for modular architecture
"""
...
class PIPDFReportGenerator:
    ...
    def _create_title_page(self, data: Dict) -> list:
        ...
        completion_statuses = results.get('completion_statuses', [])
        in_progress_statuses = results.get('in_progress_statuses', [])
        ...
        if completion_statuses:
            ...
            content.append(Paragraph("<b>Completion Statuses:</b>", self.styles['Normal']))
            statuses_text = ', '.join(completion_statuses)
            content.append(Paragraph(statuses_text, self.styles['Normal']))
        
        if in_progress_statuses:
            content.append(Spacer(1, 0.1*inch))
            content.append(Paragraph("<b>In Progress Statuses:</b>", self.styles['Normal']))
            in_prog_text = ', '.join(in_progress_statuses)
            content.append(Paragraph(in_prog_text, self.styles['Normal']))
    ...
    def _create_executive_summary(self, data: Dict) -> list:
        ...
        content.append(Paragraph("<b>Projects Analyzed:</b>", self.styles['Normal']))
        content.append(Spacer(1, 0.1*inch))
        projects_text = '<br/>'.join([f"• {project}" for project in projects])
        content.append(Paragraph(projects_text, self.styles['Normal']))
    ...
    def _create_detailed_analysis(self, data: Dict) -> list:
        ...
        if results.get('has_flow_metrics') and results.get('flow_metrics'):
            ...
            if flow_metrics:
                ...
                # Add flow metrics explanation from config
                content.append(Spacer(1, 0.2*inch))
                content.append(Paragraph("<b>Flow Metrics Reference:</b>", self.heading_style))
                content.append(Spacer(1, 0.1*inch))
                
                # Get explanations from config if available
                flow_config = results.get('flow_metrics_config', {})
                explanations = flow_config.get('Explanation', {})
                
                if explanations:
                    for metric, explanation in explanations.items():
                        metric_text = f"<b>{metric}:</b> {explanation}"
                        content.append(Paragraph(metric_text, self.styles['Normal']))
                        content.append(Spacer(1, 0.1*inch))
                else:
