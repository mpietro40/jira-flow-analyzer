...
class PDFReportGenerator:
    ...
    def _create_title_page(self, data: Dict) -> list:
        ...
        # People involvement summary
        people_involvement = data.get('people_involvement', {})
        if people_involvement and people_involvement.get('overall'):
            overall = people_involvement['overall']
            content.append(Paragraph(f"<b>People Involvement:</b> {overall.get('total_people', 0)} total people ({overall.get('reporters', 0)} reporters, {overall.get('assignees', 0)} assignees, {overall.get('commenters', 0)} commenters)", self.styles['Normal']))
            content.append(Spacer(1, 0.15*inch))
    ...
    def _create_detailed_analysis(self, data: Dict) -> list:
        ...
        # People involvement analysis
        people_involvement = data.get('people_involvement', {})
        if people_involvement and people_involvement.get('overall'):
            content.append(Spacer(1, 0.3*inch))
            content.append(Paragraph("People Involvement Analysis", self.heading_style))
            
            overall = people_involvement['overall']
            people_data = [
                ['Metric', 'Count'],
                ['Total People', str(overall.get('total_people', 0))],
                ['Reporters', str(overall.get('reporters', 0))],
                ['Assignees', str(overall.get('assignees', 0))],
                ['Commenters', str(overall.get('commenters', 0))]
            ]
            
            # Use 75% of page width for people table
            page_width = A4[0] - 144  # Subtract margins (72 each side)
            table_width = page_width * 0.75
            col1_width = table_width * 0.6
            col2_width = table_width * 0.4
            people_table = Table(people_data, colWidths=[col1_width, col2_width])
            ...
            content.append(people_table)
            
            # Add by-project breakdown if available
            by_project = people_involvement.get('by_project', {})
            if by_project:
                content.append(Spacer(1, 0.2*inch))
                content.append(Paragraph("People Involvement by Project:", self.subheading_style))
                
                project_data = [['Project', 'Total People', 'Reporters', 'Assignees', 'Commenters']]
                for project, data in by_project.items():
                    project_data.append([
                        project,
                        str(data.get('total_people', 0)),
                        str(data.get('reporters', 0)),
                        str(data.get('assignees', 0)),
                        str(data.get('commenters', 0))
                    ])
                
                # Use full page width for project table
                page_width = A4[0] - 144  # Subtract margins (72 each side)
                col_width = page_width / 5
                project_table = Table(project_data, colWidths=[col_width] * 5)
