...
class DataAnalyzer:
    ...
    def analyze_issues(self, issues: List[Dict], months_back: int = 3) -> Dict:
        ...
        return {
            ...
            'issues_with_lead_times': self._get_issues_with_lead_times(df_filtered, lead_times),
    ...
    def _calculate_people_involvement(self, issues: List[Dict], df: pd.DataFrame) -> Dict:
        ...
        for _, row in df.iterrows():
            ...
            logger.debug("Reporter for issue %s: %s", issue_key, reporter)
    ...
    def _get_issues_with_lead_times(self, df: pd.DataFrame, lead_times: List[float]) -> List[Dict]:
        """
        Get issue details corresponding to calculated lead times.
        
        Args:
            df (pd.DataFrame): DataFrame with issue data
            lead_times (List[float]): Calculated lead times
            
        Returns:
            List[Dict]: List of issues with their details
        """
        issues_with_lead_times = []
        lead_time_index = 0
        
        for _, issue in df.iterrows():
            # Check if this issue has a lead time calculated
            in_progress_date = None
            done_date = None
            
            for transition in issue['status_transitions']:
                if self._is_status_type(transition['to_status'], 'in_progress') and not in_progress_date:
                    in_progress_date = transition['changed']
                if self._is_status_type(transition['to_status'], 'done'):
                    done_date = transition['changed']
            
            if in_progress_date and done_date and lead_time_index < len(lead_times):
                issues_with_lead_times.append({
                    'key': issue['key'],
